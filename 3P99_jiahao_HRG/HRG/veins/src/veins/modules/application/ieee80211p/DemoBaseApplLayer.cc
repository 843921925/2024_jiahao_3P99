...
void DemoBaseApplLayer::handleLowerMsg(cMessage* msg)
{
    ...
    if (DemoSafetyMessage* bsm = dynamic_cast<DemoSafetyMessage*>(wsm)) {
        ...
        receivedRMs++;
        onRM(rm);
        //std::cout << "******RM recevied by _low:" << rm->getSenderAddress()<<"    time:"<<simTime()<< std::endl;
        //delete (rm);
        }

    else if (requestM* reqm = dynamic_cast<requestM*>(wsm)) {
            receivedReqMs++;
            onReqM(reqm);
            //std::cout << "------reqM recevied by _low:" << reqm->getSenderAddress()<<"    time:"<<simTime()<< std::endl;
            }
