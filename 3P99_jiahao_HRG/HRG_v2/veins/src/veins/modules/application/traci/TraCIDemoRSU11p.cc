...
//            populateWSM(actCacul);
            ...
            pool.setPa(10, 0.5,  10,0.5,  10,0.5);


//            scheduleAt(simTime() + 1200 + uniform(0.01, 0.2), actCacul);
...
//request in pool
              ...
              pool.requestKValue[pool.reqCount].push_back(pool.k1);
              pool.requestKValue[pool.reqCount].push_back(pool.k2);
              pool.requestKValue[pool.reqCount].push_back(pool.k3);
              ...
              if(slotReqCount == 100){
                  requestM* actCacul = new requestM("caculateFalseRate");
                  populateWSM(actCacul);
                  scheduleAt(simTime() + uniform(0.01, 0.2), actCacul);
              }
              pool.reqCount++;
              slotReqCount++;
              }
            ...
            for (auto it = pool.requestToVehicles.begin(); it != pool.requestToVehicles.end(); ) {
                if (it->second.find(leaveId) != it->second.end()) {
                  ...
                  // std::cout <<"get message!!!!!!!!!!!!!!!**************"<< std::endl;
                    ...
                    tempId=requestID;
                    falseCount++;
                    pool.k1_LostHistory.push_back(pool.requestKValue[requestID][0]);
                    pool.k2_LostHistory.push_back(pool.requestKValue[requestID][1]);
                    pool.k3_LostHistory.push_back(pool.requestKValue[requestID][2]);
                    pool.requestKValue.erase(requestID);
...
//        scheduleAt(simTime() +1200 +uniform(0.01, 0.2), cacul);
...
//        std::cout <<"--------------------------------------"<<std::endl;
        if(oldFalseR != 0){
            if(oldFalseR > falseRate){
                ...
                double a = 0.3;   //update rate
                double x1 = 0, y1 = 0, x2 = 0, y2 = 0, x3 = 0, y3 = 0;
                double x100 = 10.0, y100 = 0.5, x200 = 10.0, y200 =0.5, x300 = 10.0, y300 = 0.5;
...
//               std::cout << "k2_mean20: "<< pool.mean20 <<std::endl;
//               std::cout << "k3_mean30: "<< pool.mean30 <<std::endl;
               pool.mean20 = x2;
               pool.mean30 = x3;
...
//
//               std::cout << "Gra1: "<<x1-pool.x10<<std::endl;
//               std::cout << "Gra2: "<<x2-pool.x20<<std::endl;
//               std::cout << "Gra3: "<<x3-pool.x30<<std::endl;
...
//               std::cout <<"-----------------------"<<std::endl;
               ...
               pool.k1_LostHistory.clear();
               pool.k2_LostHistory.clear();
               pool.k3_LostHistory.clear();
               pool.requestToVehicles.clear();
//                x1 = pool.calculateMean(pool.k1_LostHistory);
//                y1 = pool.calculateStdDev(pool.k1_LostHistory);
//                double Gra1 = x1-pool.calculateMean(pool.k1_history);
//                double Gra10 = y1-pool.calculateStdDev(pool.k1_history);
//
////                if(std::abs(Gra1) > 0.007 ){
//////                    x100 = pool.x10-a*Gra1/pool.y10/2;
////                    x100 = pool.x10-a*(Gra1);
//////                    y100 = pool.y10+b;
////                }
////                else{
////                   x100 = pool.x10;
...
//                }
//
//                x2 = pool.calculateMean(pool.k2_LostHistory);
//                y2 = pool.calculateStdDev(pool.k2_LostHistory);
//                double Gra2 = x2-pool.calculateMean(pool.k2_history);
//                double Gra20 = y2-pool.calculateStdDev(pool.k2_history);
//
////                if(std::abs(Gra2) > 0.007){
//////                    x200 = pool.x20-a*Gra2/pool.y20/2;
////                    x200 = pool.x20-a*(Gra2);
//////                    y200 = pool.y20+b;
////                }
////                else{
////                   x200 = pool.x20;
...
//
////                x200 = pool.x20-a*(Gra2);
////                y200 = pool.y20+b*(Gra20);
////                y200 = pool.y20-0.1*a*(y2-pool.y20);
//
//                if(x200 < 0){
//                    x200 = 1;
//                }
...
//
//
//                x3 = pool.calculateMean(pool.k3_LostHistory);
//                y3 = pool.calculateStdDev(pool.k3_LostHistory);
//                double Gra3 = x3-pool.calculateMean(pool.k3_history);
//                double Gra30 = y3-pool.calculateStdDev(pool.k3_history);
//
////                if(std::abs(Gra3) > 0.007){
//////                    x300 = pool.x30-a*Gra3/pool.y30/2;
////                    x300 = pool.x30-a*(Gra3);
//////                    y300 = pool.y30+b;
////                }
////                else{
////                   x300 = pool.x30;
...
//
////                x300 = pool.x30-a*(Gra3);
////                y300 = pool.y30+b*(Gra30);
////                y300 = pool.y30-0.1*a*(y3-pool.y30);
//
//                if(x300 < 0){
//                    x300 = 1;
//                }
...
//
////print k mean and SD
//                            std::cout << "k1_mean: "<< pool.calculateMean(pool.k1_history) <<std::endl;
//                            std::cout << "k2_mean: "<< pool.calculateMean(pool.k2_history) <<std::endl;
//                            std::cout << "k3_mean: "<< pool.calculateMean(pool.k3_history) <<std::endl;
//                            std::cout << "k1_SD: "<< pool.calculateStdDev(pool.k1_history) <<std::endl;
//                            std::cout << "k2_SD: "<< pool.calculateStdDev(pool.k2_history) <<std::endl;
//                            std::cout << "k3_SD: "<< pool.calculateStdDev(pool.k3_history) <<std::endl;
//
//                            std::cout << "lossing_k1_mean: "<< x1 <<" Gra1: "<<Gra1<<std::endl;
//                            std::cout << "lossing_k2_mean: "<< x2 <<" Gra2: "<<Gra2<<std::endl;
//                            std::cout << "lossing_k3_mean: "<< x3 <<" Gra3: "<<Gra3<<std::endl;
//                            std::cout << "lossing_k1_SD: "<< y1 <<std::endl;
//                            std::cout << "lossing_k2_SD: "<< y2 <<std::endl;
//                            std::cout << "lossing_k3_SD: "<< y3 <<std::endl;
...
//                               std::cout << "k3_mean: "<< x3 <<std::endl;
//
////                               std::cout << "lossing_k1_mean: "<< pool.calculateMean(pool.k1_LostHistory) <<std::endl;
////                               std::cout << "lossing_k2_mean: "<< pool.calculateMean(pool.k2_LostHistory) <<std::endl;
////                               std::cout << "lossing_k3_mean: "<< pool.calculateMean(pool.k3_LostHistory) <<std::endl;
////
////                               std::cout <<"new_pa: ("<< x100 <<", "<<y100<<",    "<<x200 <<", "<<y200<<",    "<<x300 <<", "<<y300<<")"<<std::endl;
//                               std::cout << "new_Pa(x1,y1): "<< x100 <<"," << y100 <<std::endl;
//                               std::cout << "new_Pa(x2,y2): "<< x200 <<"," << y200 <<std::endl;
...
//            }
            ...
            pool.mean20 = pool.calculateMean(pool.k2_history);
            pool.mean30 = pool.calculateMean(pool.k3_history);
